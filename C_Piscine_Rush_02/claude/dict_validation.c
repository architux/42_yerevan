...
/*   Updated: 2025/09/14 15:38:41 by mvasilev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	validate_dictionary(t_mapping *mappings, int count)
{
	...
	if (!check_duplicate_keys(mappings, count))
		return (0);
	required = parse_dictionary(".default.dict", &req_count);
	if (!required)
		return (0);
	valid = check_required_mappings(mappings, count, required, req_count);
